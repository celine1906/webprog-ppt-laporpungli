...
@section('content')
<div class="navbars" style="background-image: url('{{ asset('images/navbar-image.png') }}'); filter: blur(2px); width: 100vw;"></div>
...
<div class="status-body" style="background-image: url('{{ asset('images/bg.png') }}');">
  <div class="bg-box">
    ...
    @foreach ($aduans as $aduan)
        <a href="{{ route('detailStatus', ['id' => $aduan->id_aduan]) }}" style="display: block; width: 1191px; height: 71px; left: 132px; top: {{ $loop->index * 71 + 242 }}px; position: absolute; text-decoration: none; color: inherit;">
            <div style="width: 100%; height: 100%; background: rgba(217, 217, 217, 0.28); border: 1px white solid; display: flex; align-items: center; justify-content: space-between; padding: 0 20px; box-sizing: border-box;">
                ...
                <div style="color: white; font-size: 24px; font-weight: 400; flex: 1; text-align: left; padding-left: 20px;">{{ $aduan->pesan }}</div>
                <div style="color: #FEF0D5; font-size: 16px; font-weight: 400;">{{ \Carbon\Carbon::parse($aduan->tanggal_kejadian)->format('d/m/Y') }}</div>
            </div>
        </a>
    @endforeach
  </div>
